...
def main():
    ...
    with dai.Device(pipeline) as device:
        ...
        #print(model[names])
        model['model'].names = [name for _, name in sorted(model['model'].names.items())]
        stride, names = model['model'].stride, model['model'].names
        ...
        while True:
            ...
            for i, det in enumerate(pred):
                ...
                if len(det):
                    ...
                    for det_row in det:
                        x1, y1, x2, y2, conf, cls = det_row[:6] 
